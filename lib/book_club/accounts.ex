defmodule BookClub.Accounts do
  ...
  alias BookClub.Accounts.{User, UserToken, UserNotifier}
  ...
  See `BookClub.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `BookClub.Accounts.User.password_changeset/3` for a list of supported options.
  ...
  def subscribe_users(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(BookClub.PubSub, "user:#{key}:users")
  end

  defp broadcast(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(BookClub.PubSub, "user:#{key}:users", message)
  end
  ...
  def list_users(%Scope{} = scope) do
    Repo.all(from user in User, where: user.user_id == ^scope.user.id)
  end
  ...
  def get_user!(%Scope{} = scope, id) do
    Repo.get_by!(User, id: id, user_id: scope.user.id)
  end
  ...
  def update_user(%Scope{} = scope, %User{} = user, attrs) do
    true = user.user_id == scope.user.id
