defmodule BookClub.AccountsTest do
  ...
  alias BookClub.Accounts.{User, UserToken}
  ...
  describe "users" do
    ...
    test "list_users/1 returns all scoped users" do
      ...
      other_user = user_fixture(other_scope)
    ...
    test "create_user/2 with valid data creates a user" do
      ...
      assert user.user_id == scope.user.id
    end
    ...
    test "delete_user/2 deletes the user" do
      scope = user_scope_fixture()
      user = user_fixture(scope)
      assert {:ok, %User{}} = Accounts.delete_user(scope, user)
      assert_raise Ecto.NoResultsError, fn -> Accounts.get_user!(scope, user.id) end
    end

    test "delete_user/2 with invalid scope raises" do
      scope = user_scope_fixture()
      other_scope = user_scope_fixture()
      user = user_fixture(scope)
      assert_raise MatchError, fn -> Accounts.delete_user(other_scope, user) end
    end

    test "change_user/2 returns a user changeset" do
      scope = user_scope_fixture()
      user = user_fixture(scope)
      assert %Ecto.Changeset{} = Accounts.change_user(scope, user)
    end
  end
end
